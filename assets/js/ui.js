(function () {
  ...
  function selectCopySource(sourceElement) {
    if (!sourceElement) {
      return;
    }
  ...
  async function copyText(value, sourceElement) {
    if (navigator.clipboard && window.isSecureContext) {
      try {
        await navigator.clipboard.writeText(value);
        return;
      } catch {
        // Fall back to manual selection for restricted clipboard environments.
      }
    }

    selectCopySource(sourceElement);
    throw new Error("Copy not supported in this browser. Please select and copy manually.");
  }
