(function () {
  ...
  function createPlayer(config) {
    ...
    let failedAdSources = new Set();
    let failedMainSources = new Set();
    ...
    function showStableMediaFailure(message) {
      isAdPlaying = false;
      activeAd = null;
      mediaElement.pause();
      ...
      setNotice(message || "Demo media could not load. Please check demo files.", "error", true);
    ...
    function loadMediaSource(source, poster, onReady) {
      ...
      if (!hasUsableSource(normalizedSource)) {
        return false;
      }

      sourceLoadToken += 1;
      if (requestedSource === normalizedSource && mediaElement.getAttribute("src") === normalizedSource) {
        setTransitioning(false);
      ...
      mediaElement.setAttribute("src", normalizedSource);
    ...
    function loadMainMedia() {
      const mainSource = normalizeSource(currentItem && currentItem.url);

      if (!currentItem || !hasUsableSource(mainSource)) {
        emptyState.hidden = false;
        titleLabel.textContent = "Invalid media source";
        setStatus("Main Video");
        setNotice("Demo media could not load. Please check demo files.", "error", true);
        setTransitioning(false);
        showStableMediaFailure("Demo media could not load. Please check demo files.");
        return;
      }

      if (failedMainSources.has(mainSource)) {
        showStableMediaFailure("Demo media could not load. Please check demo files.");
        return;
      }
      ...
      const sourceLoaded = loadMediaSource(mainSource, currentItem.thumbnail || "", function () {
    ...
    function startAd(kind, resumeTime) {
      ...
      const adSource = normalizeSource(adConfig && adConfig.url);

      if (!adConfig || !hasUsableSource(adSource) || failedAdSources.has(adSource)) {
        clearLastError();
      ...
      const sourceLoaded = loadMediaSource(adSource, "", function () {
        updateProgress();
        updateAdUi();
    ...
    mediaElement.addEventListener("error", function () {
      if (isAdPlaying) {
        if (activeAd && hasUsableSource(activeAd.url)) {
          failedAdSources.add(normalizeSource(activeAd.url));
        }
        clearLastError();
        setNotice("Main video ready", "success", false);
        finishAd();
      ...
      const failedSource = normalizeSource(currentItem && currentItem.url);
      if (hasUsableSource(failedSource)) {
        failedMainSources.add(failedSource);
      }

      if (
        currentItem &&
        !fallbackInProgress &&
        failedSource !== "demo/main-1.mp4"
      ) {
